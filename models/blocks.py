...
class FeatureQualityEstimator(nn.Module):
    """Learn an independent per-pixel reliability map for every input frame."""
    ...
    def forward(self, features: torch.Tensor, valid_mask: torch.Tensor) -> torch.Tensor:
        ...
        quality = self.net(features.reshape(b * t, c, h, w))
        quality = torch.sigmoid(quality).reshape(b, t, 1, h, w)
        return quality * mask


class QualityWeightedFusion(nn.Module):
    """Normalize learned quality over time and fuse one feature scale."""
    ...
    def forward(
        ...
        fused = (features * local_weights).sum(dim=1)
        return fused, local_weights
