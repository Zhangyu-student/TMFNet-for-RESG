...
from metrics import image_metrics, tensor_to_numpy01
from setup_utils import create_dataloader, create_model
from training_utils import load_json, move_batch


def save_image(tensor: torch.Tensor, path: Path) -> None:
    array = np.round(tensor_to_numpy01(tensor) * 255.0).astype(np.uint8)
...
def evaluate(config: Dict) -> None:
    ...
    totals = {"psnr": 0.0, "ssim": 0.0, "sam": 0.0, "mae": 0.0}
    count = 0
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing"):
            ...
            for index, name in enumerate(batch["path"]):
                ...
                save_image(output[index], pred_dir / file_name)
                save_image(batch["gt_image"][index], gt_dir / file_name)
                scores = image_metrics(output[index], batch["gt_image"][index])
                row: Dict[str, object] = {"name": file_name, **scores, "T": int(batch["valid_mask"][index].sum())}
                rows.append(row)
                for key in totals:
                ...
                if config.get("save_temporal_weights", True):
                    ...
                    weights = aux["weights_full"][index, :valid_t, 0]
    ...
    with (output_dir / "metrics.csv").open("w", newline="", encoding="utf-8-sig") as handle:
        fieldnames = ["name", "T", "psnr", "ssim", "sam", "mae"]
